...
function ImportCtr( value )
	...
	elseif value == "ASP" then --Amroth Silver Piece
		if _G.ASPWhere == 1 then	import (AppCtrD.."AmrothSilverPiece"); ASP["Ctr"]:SetPosition( _G.ASPLocX, _G.ASPLocY ); end
		if _G.ASPWhere ~= 3 then
			UpdateAmrothSilverPiece();
	end
	elseif value == "SOM" then --Stars of Merit
		if _G.SOMWhere == 1 then	import (AppCtrD.."StarsofMerit"); SOM["Ctr"]:SetPosition( _G.SOMLocX, _G.SOMLocY ); end
		if _G.SOMWhere ~= 3 then
			UpdateStarsofMerit();
	end
	elseif value == "CGSP" then --Central Gondor Silver Piece
		if _G.CGSPWhere == 1 then	import (AppCtrD.."CentralGondorSilverPiece"); CGSP["Ctr"]:SetPosition( _G.CGSPLocX, _G.CGSPLocY ); end
		if _G.CGSPWhere ~= 3 then
			UpdateCentralGondorSilverPiece();
	end
	elseif value == "GGB" then --Gift giver's Brand
		if _G.GGBWhere == 1 then	import (AppCtrD.."GiftgiversBrand"); GGB["Ctr"]:SetPosition( _G.GGBLocX, _G.GGBLocY ); end
		if _G.GGBWhere ~= 3 then
			UpdateGiftgiversBrand();
	end
	-- AU3 MARKER END
...
function LoadPlayerReputation()
	RepFR = { "La confr\195\168rie de la cervoise", "Les Eldgangs", "Conseil du Nord", "Hommes de Bree", "Palais de Thorin", "Les Gardiens d'Annuminas", "Lossoth du Forochel", "Les Eglain", 
	"R\195\180deurs d'Esteld\195\173n", "Elfes de Foncombe", "La Soci\195\169t\195\169 des Mathoms", "La Ligue des Tavernes", "Gardes de la Garnison de fer", "Minuers de la Garnison de fer", 
	"Les Algraig, Hommes d'Enedwaith", "La Compagnie grise", "Galadhrim", "Malledhrim", "Les Cavaliers de Stangarde", "Les H\195\169ros de la Gorge de Limeclair", "Hommes du Pays de Dun", 
